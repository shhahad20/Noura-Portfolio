...
const IlluminatedHero: React.FC<IlluminatedHeroProps> = ({
  title = "Introducing",
  subtitle = "Highlight the main focus text.",
  glowText = "Illuminated Glow Text.",
  description = "Experience a new way to draw attention to key elements with stunning",
  highlightText = "illuminated text.",
  className = ""
}) => {
//   const fullDescription = `${description} ${highlightText} Perfect for making a bold statement, this dynamic design ensures your content stands out effortlessly.`;

  return (
    <section className={`illuminated-hero ${className}`}>
      {/* Background glow elements */}
      <div className="illuminated-hero__bg">
        <div></div>
        <div></div>
      </div>

      {/* Main header text */}
      <div className="illuminated-hero__header" aria-hidden="true">
        {title}<br />
        <span className="illuminated-hero__glow-text" data-text={glowText}>
          {glowText}
        </span><br />
        {subtitle}<br />
      </div>
      ...
      <p className="illuminated-hero__description">
        {description} <span className="illuminated-hero__highlight">{highlightText}.</span> Perfect for making a bold statement, this dynamic design ensures your content stands out effortlessly.
      </p>

      {/* SVG filters for the glow effect */}
      <svg 
        className="illuminated-hero__filters" 
        width="1440" 
        height="300" 
      ...
      >
        <defs>
          <filter id="illuminated-glow-4" colorInterpolationFilters="sRGB" x="-50%" y="-200%" width="200%" height="500%">
